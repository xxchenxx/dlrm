...
def run():
    ...
    parser.add_argument("--grad-noise", type=float, default=0.1)
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    with record_function("DLRM backward"):
                        ...
                        for name, p in dlrm.named_parameters():
                            if not 'emb' in name:
                                p.grad.data.add_(torch.randn(p.data.shape).to(p.data.device) * args.grad_noise)
                    ...
                    if should_print or should_test:
                        ...
                        hei = hessian_eigen_input(dlrm, train_ld, dlrm_wrap, loss_fn_wrap, False, 5, use_gpu=use_gpu, ndevices=ndevices)[0][0]
                        ...
                        ht = hessian_trace(dlrm, train_ld, dlrm_wrap, loss_fn_wrap, False, 5, use_gpu=use_gpu, ndevices=ndevices)[0]
